module.exports = [
  {
    id: 1,
    imgIcon: "img/svg/1.svg",
    title: (
      <>
        Çocuk ve Oyun <br /> Terapisi
      </>
    ),
    popupImg: "img/service/1.jpg",
    popupTitle: `Çocuk ve Oyun Terapisi`,
    firstDescriptionText: <></>,
    serviceListDescriptions: (
      <>
        <h3>Çocuk ve Oyun Terapisi</h3>
        <p>Çocuk ve Oyun Terapisi soyledır boyledır cok onemlıdır</p>
        <div className="in_list">
          <ul>
            <li>
              <p>Alt Islatma</p>
            </li>
            <li>
              <p>Bağlanma Sorunları</p>
            </li>
            <li>
              <p>Öfke Sorunları</p>
            </li>
            <li>
              <p>Kaygı Sorunları</p>
            </li>
            <li>
              <p>Tırnak Yeme</p>
            </li>
            <li>
              <p>Kardeş Kıskançlığı</p>
            </li>
            <li>
              <p>Ebeveynlerin Boşanma Süreci</p>
            </li>
            <li>
              <p>Travma</p>
            </li>
            <li>
              <p>Takıntılar</p>
            </li>
            <li>
              <p>Ayrılık Kaygısı</p>
            </li>
            <li>
              <p>Okul Reddi</p>
            </li>
            <li>
              <p>Yeme Sorunları</p>
            </li>
            <li>
              <p>Karşıt Olma / Karşıt Gelme</p>
            </li>
            <li>
              <p>Hiperaktivite</p>
            </li>
            <li>
              <p>Disleksi, Diskalkuli</p>
            </li>
            <li>
              <p>Çocuk Değerlendirme Testleri</p>
            </li>
            <li>
              <p>WISCR Zeka Testi</p>
            </li>
          </ul>
        </div>
        {/* End list */}
        <p>
          Bu konularda tecrubelerımle onlıne ve yuzyuze olarak terapıler
          yapmaktayım.
        </p>
      </>
    ),
  },
  {
    id: 2,
    imgIcon: "img/svg/2.svg",
    title: (
      <>
        Ergenlerle <br /> Terapi
      </>
    ),
    popupImg: "img/service/2.jpg",
    popupTitle: `
    Ergenlerle Terapi`,
    firstDescriptionText: <></>,
    serviceListDescriptions: (
      <>
        <h3>Ergenlerle Terapi</h3>
        <p>Ergenlerle Terapinin onemi ve anlami analtilacak.</p>
        <div className="in_list">
          <ul>
            <li>
              <p>Sınav Kaygısı</p>
            </li>
            <li>
              <p>Obsesif Kompülsif Bozukluk</p>
            </li>
            <li>
              <p>Teknoloji Bağımlılığı</p>
            </li>
            <li>
              <p>Sigara Bağımlılığı</p>
            </li>
            <li>
              <p>Okul Problemleri</p>
            </li>
            <li>
              <p>Okul Başarısızlığı</p>
            </li>
            <li>
              <p>Sosyal Fobi</p>
            </li>
            <li>
              <p>Ergen Birey ile Aile İşbirliğinin Sağlanması</p>
            </li>
            <li>
              <p>Ergenlikte Öfke ve Saldırganlık</p>
            </li>
            <li>
              <p>Ergenlik Dönemi Depresyonu</p>
            </li>
            <li>
              <p>Arkadaş Problemleri</p>
            </li>
          </ul>
        </div>
        {/* End list */}
        <p>Ergenlerle Terapi konusunda tecrubeler ve metodlar anlatilacak</p>
      </>
    ),
  },
  {
    id: 3,
    imgIcon: "img/svg/3.svg",
    title: (
      <>
        Yetişkinlerle <br />
        Terapi
      </>
    ),
    popupImg: "img/service/3.jpg",
    popupTitle: `Yetişkinlerle Terapi`,
    firstDescriptionText: <></>,
    serviceListDescriptions: (
      <>
        <h3>Yetişkinlerle Terapi</h3>
        <p>Yetişkinlerle Terapinin onemi ve anlami anlatilacak</p>
        <div className="in_list">
          <ul>
            <li>
              <p>Depresyon Bozuklukları</p>
            </li>
            <li>
              <p>Anksiyete Bozuklukları</p>
            </li>
            <li>
              <p>Takıntı – Zorlantı Bozukluğu (Obsesif – Kompulsif Bozukluk)</p>
            </li>
            <li>
              <p>
                Örselenme (Travma) ve Tetikleyici Etkenle (Stresörle) İlişkili
                Bozukluklar
              </p>
            </li>
            <li>
              <p>Yıkıcı Bozukluklar, Dürtü Denetimi ve Davranım Bozuklukları</p>
            </li>
            <li>
              <p>İlişki Sorunları</p>
            </li>
            <li>
              <p>Bedensel Belirti Bozuklukları ve İlişkili Bozukluklar</p>
            </li>
            <li>
              <p>Beslenme ve Yeme Bozuklukları</p>
            </li>
            <li>
              <p>
                Sigara Alkol ve Teknoloji Bağımlılıkları ile İlişkili
                Bozukluklar
              </p>
            </li>
            <li>
              <p>İletişim Becerileri, Öfke ve Stres Denetimi</p>
            </li>
            <li>
              <p>Beslenme ve Yeme Bozuklukları</p>
            </li>
            <li>
              <p>Toplumsal Çevre ile İlişkili Diğer Sorunlar</p>
            </li>
          </ul>
        </div>
        {/* End list */}
        <p>Yetişkinlerle Terapiyle ilgili tecrubeler ve yontemler anlatilacak</p>
      </>
    ),
  },
  {
    id: 4,
    imgIcon: "img/svg/4.svg",
    title: (
      <>
        Mobile App <br /> Design
      </>
    ),
    popupImg: "img/service/4.jpg",
    popupTitle: `Mobile App Design is the trending Part of Graphics`,
    firstDescriptionText: (
      <>
        <p>
          Just because we cant get out and about like we normally would, doesn’t
          mean we have to taking pictures. There’s still plenty you can do,
          provided you are prepared to use some imagination. Here are a few
          ideas to keep you shooting until normal life resumes.
        </p>
        <p>
          Most photographers love to shoot the unusual, and you don’t get much
          more unusual than These Unprecedented Times. Right now everything
          counts as out of the ordinary. There are a number of remarkable things
          about these lockdown days that are worth photographing now we can
          remember them when it is all over.
        </p>
      </>
    ),
    serviceListDescriptions: (
      <>
        <h3>What Is Mobile App Design?</h3>
        <p>
          Nullam blandit constituto eam ne, te nam ignota vituper.atoribus.
          Malis cetero quem saipe, prou thendrerit voluptaria.
        </p>
        <div className="in_list">
          <ul>
            <li>
              <p>Email is a crucial channel in any marketing.</p>
            </li>
            <li>
              <p>Curious what to say? How to say it?</p>
            </li>
          </ul>
        </div>
        {/* End list */}
        <p>
          Streets empty that are usually busy are remarkable and can evoke the
          sense of historical pictures from before the invention of the
          motorcar. Other things that are different at the moment will be queues
          to get into stores and the lines marked out on the floor to show how
          far apart we should be.
        </p>
      </>
    ),
  },
];
